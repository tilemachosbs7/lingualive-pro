...
export class MicRecorder {
  private readonly options: MicRecorderOptions;
  ...
  async start(): Promise<void> {
    ...
    console.log(
      `[MicRecorder] Started recording with mimeType: ${this.mediaRecorder.mimeType}`
    );
  }
  ...
  private async handleDataAvailable(event: BlobEvent): Promise<void> {
    ...
    try {
      ...
      const error =
        err instanceof Error ? err : new Error("Failed to read audio chunk");
      this.options.onError?.(error);
    }
  }
}
