...
import { fetchUserBootstrap, fetchUserLimitsSummary, fetchUserUsageSummary, } from "../api/backendClient";
...
let container = null;
let statusEl = null;
let sessionConfigEl = null;
let planBadgeEl = null;
let limitsInfoEl = null;
let usageInfoEl = null;
let startBtn = null;
let stopBtn = null;
let errorEl = null;
let transcriptEl = null;
let closeBtn = null;
let upgradeNudgeEl = null;
let smartWarningEl = null;
let authRequiredEl = null;
let openSettingsBtn = null;
let limitsExceededEl = null;
let limitsExceededUpgradeBtn = null;
 ...
 */
function setError(message, showDiagnosticsLink = false) {
    if (!errorEl)
        return;
    if (message) {
      ...
      `;
            ...
            const diagLink = errorEl.querySelector("#llp-diag-link");
            if (diagLink) {
...
function showAuthRequired(message) {
    ...
    const textEl = authRequiredEl.querySelector("#llp-auth-message");
    if (textEl) {
 ...
 */
function isAuthError(error, statusCode) {
 ...
 */
function showLimitsExceeded(message) {
    if (!limitsExceededEl)
        return;
    const defaultMessage = t('hud.limits.reached');
    const messageEl = limitsExceededEl.querySelector("#llp-limits-message");
    if (messageEl) {
 ...
 */
function handleTabChunkMessage(message) {
    const m = message;
    if (m.type !== "LLP_TAB_AUDIO_CHUNK")
        return;
    if (!client || !m.buffer || typeof m.timestampMs !== "number")
        return;
    try {
...
function runtimeMessageListener(message) {
    ...
    if (typeof message === "object" &&
        message !== null &&
        message.type === "LLP_TOGGLE_CAPTIONS") {
        void toggleCaptions();
        return;
    }
...
async function handleStart() {
    ...
    const qualityMode = settings.defaultQualityMode ?? settings.preferredQualityMode ?? "FAST";
...
function injectHud() {
  ...
  `;
    ...
    // Grab references
    statusEl = container.querySelector("#llp-status");
    sessionConfigEl = container.querySelector("#llp-session-config");
    planBadgeEl = container.querySelector("#llp-plan-badge");
    limitsInfoEl = container.querySelector("#llp-limits-info");
    usageInfoEl = container.querySelector("#llp-usage-info");
    upgradeNudgeEl = container.querySelector("#llp-upgrade-nudge");
    smartWarningEl = container.querySelector("#llp-smart-warning");
    startBtn = container.querySelector("#llp-start-btn");
    stopBtn = container.querySelector("#llp-stop-btn");
    errorEl = container.querySelector("#llp-error");
    transcriptEl = container.querySelector("#llp-transcript");
    closeBtn = container.querySelector("#llp-close-btn");
    authRequiredEl = container.querySelector("#llp-auth-required");
    openSettingsBtn = container.querySelector("#llp-open-settings-btn");
    limitsExceededEl = container.querySelector("#llp-limits-exceeded");
    limitsExceededUpgradeBtn = container.querySelector("#llp-limits-upgrade-btn");
    ...
    const audioSourceInputs = container.querySelectorAll('input[name="llp-audio-source"]');
    ...
    if (upgradeNudgeEl) {
        ...
        upgradeNudgeEl.addEventListener("keydown", (e) => {
    ...
    if (openSettingsBtn) {
        ...
        openSettingsBtn.addEventListener("keydown", (e) => {
            if (e.key === "Enter" || e.key === " ") {
                e.preventDefault();
                openOptionsPage();
            }
