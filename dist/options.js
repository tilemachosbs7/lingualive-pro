...
import { fetchUserBootstrap, fetchUserLimitsSummary, fetchPublicStatus, requestDevToken, revokeDevToken, } from "./api/backendClient";
import { RealtimeClient } from "./api/realtimeClient";
import { t, setLocale } from "./i18n/messages";
// =============================================================================
// PREFERENCES ELEMENTS
// =============================================================================
let sourceLangInput = null;
let targetLangInput = null;
...
let devTokenInput = null;
let debugUserIdInput = null;
let debugPlanInput = null;
let testConnectionBtn = null;
let connectionTestStatusEl = null;
let generateDevTokenBtn = null;
let generateDevTokenStatusEl = null;
let revokeDevTokenBtn = null;
let devTokenExpiryEl = null;
// Summary panel elements
let summaryPlanEl = null;
let summaryUsageEl = null;
let summaryUpgradeBtn = null;
let summaryErrorEl = null;
let summaryNoAuthEl = null;
let summaryContentEl = null;
let summaryLimitStatusEl = null;
// Limit status elements
let audioLimitStatusEl = null;
let translationLimitStatusEl = null;
// Diagnostics elements
let runDiagnosticsBtn = null;
let diagnosticsResultsEl = null;
let diagnosticsListEl = null;
let copyDiagnosticsBtn = null;
 ...
 */
function _calculateLimitStatus(usage, limit) {
    // No limit = always OK
 ...
 */
async function checkRealtimeWebSocket(settings) {
    ...
    try {
        ...
        const connectWithTimeout = Promise.race([
            client.connect(),
            new Promise((_, reject) => setTimeout(() => reject(new Error("WebSocket connection timeout")), timeoutMs)),
        ]);
 ...
 */
function renderDiagnosticsResults(report) {
 ...
 */
function generateDiagnosticsText(report) {
...
async function handleRunDiagnostics() {
    if (!runDiagnosticsBtn || !diagnosticsListEl)
        return;
...
async function handleCopyDiagnostics() {
    if (!copyDiagnosticsBtn || !copyStatusEl)
        return;
...
function setVisible(id, visible) {
    byId(id).style.display = visible ? "block" : "none";
}
// =============================================================================
// UI UPDATE HELPERS
// =============================================================================
...
function updateAccountSection(bootstrap, limits) {
    ...
    setText("allowed-quality-modes", modes.length > 0 ? modes.join(", ") : "None");
}
/**
 ...
 */
function updateLimitsSection(limits) {
    // Audio limit
    const audioLimit = limits.per_session_audio_limit_minutes;
    setText("audio-limit", audioLimit == null ? "Unlimited" : `${audioLimit.toFixed(1)} minutes`);
    ...
    setText("translation-limit", translationLimit == null
        ? "Unlimited"
 ...
 */
function updateSummaryLimitStatus(limits) {
    if (!summaryLimitStatusEl)
        return;
 ...
 */
function updateUsageSection(bootstrap) {
    ...
    setText("total-audio-minutes", isNaN(audioMinutes) ? usage.audio_minutes_total : audioMinutes.toFixed(2));
    // Transcript chars
 ...
 */
function showSummaryNoAuth() {
    if (summaryContentEl)
        summaryContentEl.style.display = "none";
    if (summaryErrorEl)
        summaryErrorEl.style.display = "none";
    if (summaryNoAuthEl)
        summaryNoAuthEl.style.display = "block";
}
/**
 ...
 */
function renderSummaryPanel(bootstrap) {
    if (!summaryPlanEl || !summaryUsageEl)
        return;
...
function applyI18nToStaticElements() {
    ...
    if (diagnosticsTitleEl)
        diagnosticsTitleEl.textContent = t('options.diagnostics.title');
    const diagnosticsDescEl = document.getElementById("diagnostics-description");
    if (diagnosticsDescEl)
        diagnosticsDescEl.textContent = t('options.diagnostics.description');
    // Buttons
    if (runDiagnosticsBtn)
        runDiagnosticsBtn.textContent = t('options.diagnostics.runButton');
    if (copyDiagnosticsBtn)
        copyDiagnosticsBtn.textContent = t('options.buttons.copyDiagnostics');
    if (testConnectionBtn)
        testConnectionBtn.textContent = t('options.buttons.testConnection');
    ...
    if (languageSelect) {
        ...
        for (let i = 0; i < options.length; i++) {
            const opt = options[i];
            if (opt.value === 'auto')
                opt.textContent = t('options.language.auto');
            else if (opt.value === 'el')
                opt.textContent = t('options.language.el');
            else if (opt.value === 'en')
                opt.textContent = t('options.language.en');
        }
    }
    // Summary no-auth message
    const summaryNoAuthEl = document.getElementById("summary-no-auth");
    if (summaryNoAuthEl)
        summaryNoAuthEl.textContent = t('options.summary.noAuth');
    // Shortcuts & controls section
    const shortcutsSectionTitleEl = document.getElementById("shortcuts-section-title");
    if (shortcutsSectionTitleEl)
        shortcutsSectionTitleEl.textContent = t('options.shortcuts.sectionTitle');
    const shortcutsHowToStartEl = document.getElementById("shortcuts-how-to-start");
    if (shortcutsHowToStartEl)
        shortcutsHowToStartEl.textContent = t('options.shortcuts.howToStart');
    const shortcutsHowToStartStepsEl = document.getElementById("shortcuts-how-to-start-steps");
    if (shortcutsHowToStartStepsEl)
        shortcutsHowToStartStepsEl.textContent = t('options.shortcuts.howToStart.steps');
    const shortcutsKeyboardHeadingEl = document.getElementById("shortcuts-keyboard-heading");
    if (shortcutsKeyboardHeadingEl)
        shortcutsKeyboardHeadingEl.textContent = t('options.shortcuts.keyboardHeading');
    const shortcutsKeyboardToggleEl = document.getElementById("shortcuts-keyboard-toggle");
    if (shortcutsKeyboardToggleEl)
        shortcutsKeyboardToggleEl.textContent = t('options.shortcuts.keyboardToggle');
    const shortcutsKeyboardOpenOptionsEl = document.getElementById("shortcuts-keyboard-open-options");
    if (shortcutsKeyboardOpenOptionsEl)
        shortcutsKeyboardOpenOptionsEl.textContent = t('options.shortcuts.keyboardOpenOptions');
    const shortcutsContextMenuHeadingEl = document.getElementById("shortcuts-context-menu-heading");
    if (shortcutsContextMenuHeadingEl)
        shortcutsContextMenuHeadingEl.textContent = t('options.shortcuts.contextMenuHeading');
    const shortcutsContextMenuDescriptionEl = document.getElementById("shortcuts-context-menu-description");
    if (shortcutsContextMenuDescriptionEl)
        shortcutsContextMenuDescriptionEl.textContent = t('options.shortcuts.contextMenuDescription');
    const shortcutsChromeCustomizeEl = document.getElementById("shortcuts-chrome-customize");
    if (shortcutsChromeCustomizeEl)
        shortcutsChromeCustomizeEl.textContent = t('options.shortcuts.chromeShortcutsNote');
    // Help & troubleshooting section
    const helpSectionTitleEl = document.getElementById("help-section-title");
    if (helpSectionTitleEl)
        helpSectionTitleEl.textContent = t('options.help.sectionTitle');
    const helpWhenToUseEl = document.getElementById("help-when-to-use");
    if (helpWhenToUseEl)
        helpWhenToUseEl.textContent = t('options.help.whenToUseDiagnostics');
    const helpStepsHeadingEl = document.getElementById("help-steps-heading");
    if (helpStepsHeadingEl)
        helpStepsHeadingEl.textContent = t('options.help.stepsHeading');
    const helpStepCheckBackendEl = document.getElementById("help-step-check-backend");
    if (helpStepCheckBackendEl)
        helpStepCheckBackendEl.textContent = t('options.help.stepCheckBackend');
    const helpStepCheckMicEl = document.getElementById("help-step-check-mic");
    if (helpStepCheckMicEl)
        helpStepCheckMicEl.textContent = t('options.help.stepCheckMic');
    const helpStepCopyReportEl = document.getElementById("help-step-copy-report");
    if (helpStepCopyReportEl)
        helpStepCopyReportEl.textContent = t('options.help.stepCopyReport');
    const helpPrivacyNoteEl = document.getElementById("help-privacy-note");
    if (helpPrivacyNoteEl)
        helpPrivacyNoteEl.textContent = t('options.help.privacyNote');
}
// =============================================================================
// INITIALIZATION
// =============================================================================
...
function initOptionsPage() {
    ...
    sourceLangInput = document.getElementById("llp-source-lang");
    targetLangInput = document.getElementById("llp-target-lang");
    qualitySelect = document.getElementById("llp-quality-mode");
    languageSelect = document.getElementById("language-select");
    saveBtn = document.getElementById("save-btn");
    saveStatusEl = document.getElementById("save-status");
    // Get backend & auth elements
    backendUrlInput = document.getElementById("llp-backend-url");
    devTokenInput = document.getElementById("llp-dev-token");
    debugUserIdInput = document.getElementById("llp-debug-user-id");
    debugPlanInput = document.getElementById("llp-debug-plan");
    testConnectionBtn = document.getElementById("test-connection-btn");
    connectionTestStatusEl = document.getElementById("connection-test-status");
    generateDevTokenBtn = document.getElementById("generate-dev-token-btn");
    generateDevTokenStatusEl = document.getElementById("generate-dev-token-status");
    revokeDevTokenBtn = document.getElementById("revoke-dev-token-btn");
    devTokenExpiryEl = document.getElementById("dev-token-expiry");
    // Get summary panel elements
    summaryPlanEl = document.getElementById("summary-plan");
    summaryUsageEl = document.getElementById("summary-usage");
    summaryUpgradeBtn = document.getElementById("summary-upgrade-btn");
    summaryErrorEl = document.getElementById("summary-error");
    summaryNoAuthEl = document.getElementById("summary-no-auth");
    summaryContentEl = document.getElementById("summary-content");
    summaryLimitStatusEl = document.getElementById("summary-limit-status");
    // Get limit status elements
    audioLimitStatusEl = document.getElementById("audio-limit-status");
    translationLimitStatusEl = document.getElementById("translation-limit-status");
    // Get diagnostics elements
    runDiagnosticsBtn = document.getElementById("run-diagnostics-btn");
    diagnosticsResultsEl = document.getElementById("diagnostics-results");
    diagnosticsListEl = document.getElementById("diagnostics-list");
...
async function handleTestConnection() {
    if (!testConnectionBtn || !connectionTestStatusEl)
        return;
 ...
 */
function renderDevTokenExpiry(settings) {
    if (!devTokenExpiryEl)
        return;
 ...
 */
async function handleGenerateDevToken() {
    if (!generateDevTokenBtn || !generateDevTokenStatusEl)
        return;
 ...
 */
async function handleRevokeDevToken() {
    if (!revokeDevTokenBtn || !generateDevTokenStatusEl)
        return;
